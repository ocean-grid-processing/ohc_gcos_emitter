...
measured layer up to the depth slab it stands in for (e.g. the 5 m `15_20` layer x3 covers the
unmeasured `0-15 m` top; the 50 m `1800_1850` sliver x3 covers `1850-2000 m`). `dz` is the
contributor's own thickness (bottom - top); `sum(n_fac*dz)` equals the nominal layer thickness.

Ported verbatim from the original WMO2024_create_tseries_with_uq_for_combined.m, plus a net-new
`0_1000` (its `700_1000` contributor is a new mapped layer). Edit LEVELS to add/remove combined
layers; `--levels` on the CLI selects a subset for a given run.
"""
